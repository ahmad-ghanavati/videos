        	...
        	<div id="upload">
                <div class="row">
                    ...
                    <div class="col-md-8">
						...
						<form action="{{ route('videos.update',$video->slug) }}" method="POST" enctype="multipart/form">
                            @csrf
                        	<div class="row">
                            	<div class="col-md-6">
                                    ...
                                    <select name="category_id" id="category" class="form-control" >
                                        @foreach ($categoris as $category)
                                            <option name="category_id"  value="{{ $category->id }}" 
                                                {{ $category->id==$video->category->id ? 'selected' : ''  }}>{{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            	<div class="col-md-6">
                                    ...
                                    <input type="text" name="length" class="form-control" value="{{ $video->length }}" placeholder="@lang('videos.timeFilm')">
                                </div>
                            	<div class="col-md-6">
                                	<label>@lang('videos.labale')</label>
                                    <input type="text" name="slug" class="form-control" value="{{ $video->slug }}" placeholder="@lang('videos.labale')">
                                </div>
                            	<div class="col-md-6">
                                	<label>@lang('videos.uploadFilm')</label>
                                    <input id="upload_file" name="url"  class="file" value="{{$video->url }}">
                                </div>
                            	<div class="col-md-12">
                                	<label>@lang('videos.description')</label>
                                    <textarea  name="description" class="form-control"  value="{{ $video->description }}" rows="4"  placeholder="@lang('videos.description')"></textarea>
                                </div>
                            	<div class="col-md-6">
                                	<label>@lang('videos.img')</label>
                                    <input id="featured_image" name="thumbnail"  value="{{ $video->thumbnail }}" class="file">
                                </div>
                            	<div class="col-md-6">
